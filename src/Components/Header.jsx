    ...
    <div className={mode && 'Home'}>
        <div className="Header z-10 fixed flex justify-center alternova-border border-b-2 bg-white px-3 h-20 rounded-b-lg items-center w-full top-0 mt-0">
            ...
            <Link to="/shopping-cart">
                ...
                {cart.length > 0 ? 
                <span className="absolute top-4 right-8 alternova-bg text-white h-4 w-4 rounded-full flex justify-center items-center text-xs font-bold"> {cart.length}</span>
              : null }
        </div>
        <ProductList />
          <Footer />
